...
class EventServiceTest {
  ...
  void get_events() {
    ...
    when(eventRepository.findBy(pageable)).thenAnswer(invocation -> {
      ...
      return Flowable.fromIterable(IntStream.range(0, invokedPageable.getPageSize()).mapToObj(i ->
          Event.from("Event " + i, EventDefaults.START_TIME.plusDays(i),
              EventDefaults.START_TIME.plusDays(i).plusHours(i + 1))).toList());
    });
  ...
  void update_event_participants() {
    ...
    when(participantRepository.saveAll(any(List.class))).thenAnswer(invocation ->
        Flowable.fromIterable(invocation.getArgument(0, List.class)));
